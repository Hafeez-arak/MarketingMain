// ─── Brand context assembly — the pure half ────────────────────────────────
// buildContext() itself, and everything it needs. This is the single entry
// point every AI call goes through to turn the Brand Brain into the text a
// model reads, and AGENT.md §3 makes it the single most important constraint
// in the agent's design: the server must go through THIS function, not a
// second assembler of its own, or the two views of the brand diverge with no
// traceable cause.
//
// It could not, before this split. brandContext.js imports React and
// supabaseClient, and supabaseClient reads `import.meta.env`, which does not
// exist outside the Vite bundle — a plain import() of it under Node fails on
// './supabaseClient'. So the function that had to be shared was the one
// function the server could not load.
//
// Nothing here imports React, a store, or the network. brandContext.js
// re-exports all of it, so no existing call site changes.

import { buildInstructionsString, buildSectionBlocks } from './brandBrainCore.js'
