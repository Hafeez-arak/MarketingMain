...
// Brand research used to live here too: requestBrandResearch() fired the
// `arak-brand-research` workflow, and competitorNamesFrom() built its payload
// out of the rival directory. Both are gone. The research agent's own run
// writes `proposed` rows into this same table with evidence attached, and a
// second code path writing brand_memory is exactly the drift buildContext
// exists to prevent. competitorRowsFrom() in research.js replaced the names
// helper — it returns the website and positioning too, which resolution needs.
