...
// ─── Reference Picker ──────────────────────────────────────────────────────
// Shared modal for attaching images to a single post (used by the campaign
// planner and the per-day schedule editors). An image is just a public URL —
// either picked from the Brand Brain asset library or uploaded fresh. Returns
// the full ordered list of selected URLs via onSave.
//
// Two jobs, chosen by the caller:
//   default   references that GUIDE an image generation (image-to-image)
//   asPost    the picked image IS the post, as-is
//
// The planner only ever uses `asPost`. It used to offer a "Generate with AI"
// choice here too, which duplicated what Creative Studio does properly — a
// plan's AI pictures are made in the Studio, and this picker is only for a
// picture you already have.

// Only visual kinds make sense as images — music can't be a picture.
const REFERENCE_KINDS = ASSET_KINDS.filter(k => k.value !== 'music')

// `format` drives the count rule for `asPost`: a single post takes exactly one
// image, a carousel takes several (its slides, in order).
export function ReferencePicker({ value = [], onSave, onClose, asPost = false, format = 'post' }) {
  ...
  const isCarousel  = format === 'carousel' || format === 'photo_carousel'
  const singleOnly  = asPost && !isCarousel
  // Opening a single-image post that somehow carries several keeps the first,
  // so the grid never paints more selections than the mode permits.
  const [selected,  setSelected]  = useState(() => singleOnly ? value.slice(0, 1) : value)
  ...
  useEffect(() => {
    ...
    fetchBrandAssets(activeWorkspaceId, accessToken).then(rows => {
      if (!alive) return
      // Only assets we can actually show — must have a URL and not be audio.
  ...
  return (
    <Modal open onClose={onClose} title={asPost ? 'Post image' : 'Reference images'} width="max-w-2xl">
      <div className="p-6 space-y-4">
        <p className="text-xs text-text-secondary leading-relaxed">
          {!asPost
            ...
            : singleOnly
              ? 'Pick the image to use as this post — it goes out as-is. Want AI to make one instead? Use Open Studio on the pictures step.'
              : 'Pick the images to use as the carousel slides, in order — they go out as-is.'}
        </p>
          ...
          <div className="space-y-3">
            ...
            {loading ? (
              ...
              <p className="text-xs text-text-tertiary text-center py-10">
                No brand assets yet. Add photos in Brand Brain → Asset Library, or upload one directly.
              </p>
            ) : (
        ...
        {tab === 'upload' && (
          <div className="space-y-3">
            <label className="flex flex-col items-center justify-center gap-2 py-10 border-2 border-dashed border-border rounded-xl cursor-pointer hover:border-amber-400 hover:bg-surface-subtle transition-colors">
              <input ref={fileRef} type="file" accept="image/*" multiple={!singleOnly} className="hidden" onChange={handleFiles} disabled={uploading} />
              {uploading ? (
                ...
                <>
                  ...
                  <span className="text-xs font-medium text-text-secondary">Click to upload {singleOnly ? 'an image' : 'images'}</span>
                  <span className="text-[11px] text-text-tertiary">PNG, JPG, WEBP</span>
                </>
              )}
            </label>
            <p className="text-[11px] text-text-tertiary">Uploads are added to the selection above automatically.</p>
          </div>
        )}
