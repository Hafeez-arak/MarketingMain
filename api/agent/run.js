...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      ok: true,
      run_id: runId,
      // Echoed so the driver reads the workspace from this reply alone. The
      // n8n workflow has two triggers, and a lens step that looked the
      // workspace up from one trigger's node would fail on the other branch.
